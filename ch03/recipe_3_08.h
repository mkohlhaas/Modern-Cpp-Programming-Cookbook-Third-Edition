...
namespace recipe_3_08
{
    ...
    execute()
    {
        ...
        {
            auto vnums = std::vector<int>{0, 2, -3, 5, -1, 6, 8, -4, 9};
            auto s1    = funclib::fold_left([](const int s, const int n) { return s + n; }, vnums, 0); // s1 = 22
            auto s2    = funclib::fold_left(std::plus<>(), vnums, 0);                                  // s2 = 22

            auto s3 = funclib::fold_right([](const int s, const int n) { return s + n; }, vnums, 0);   // s3 = 22
            auto s4 = funclib::fold_right(std::plus<>(), vnums, 0);                                    // s4 = 22

            auto texts = std::vector<std::string>{"hello"s, " "s, "world"s, "!"s};
            auto txt1 =
                funclib::fold_left([](std::string const &s, std::string const &n) { return s + n; }, texts, ""s);
            // txt1 = "hello world!"
            auto txt2 =
                funclib::fold_right([](std::string const &s, std::string const &n) { return s + n; }, texts, ""s);
            // txt2 = "!world hello"

            char chars[] = {'c', 'i', 'v', 'i', 'c'};
            auto str1    = funclib::fold_left(std::plus<>(), chars, ""s);  // str1 = "civic"
            auto str2    = funclib::fold_right(std::plus<>(), chars, ""s); // str2 = "civic"

            auto words = std::map<std::string, int>{{"one", 1}, {"two", 2}, {"three", 3}};
            auto count = funclib::fold_left(
                [](int const s, std::pair<std::string, int> const kvp) { return s + kvp.second; }, words, 0);
            ...
            q.push(4);
            q.push(5);
            auto sum = funclib::fold_left(std::plus<>(), q, 0); // sum = 15
        }

        {
            auto vnums = std::vector<int>{0, 2, -3, 5, -1, 6, 8, -4, 9};

            auto s = funclib::fold_left(std::plus<>(),
