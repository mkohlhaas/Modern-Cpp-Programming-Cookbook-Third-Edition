...
namespace recipe_3_03
{
    ...
    struct __lambda_name__
    {
        ...
        auto
        operator()(T1 const s, T2 const n) const
        {
            return s + n;
        }
    ...
    execute()
    {
        ...
        {
            auto tl = [](auto &&...args) { return foobar(std::forward<decltype(args)>(args)...); };
        ...
        // C++20 template lambdas
        {
            ...
            auto             tl = []<typename T>(std::vector<T> const &vec) { std::cout << std::size(vec) << '\n'; };
        ...
        {
            auto tl = []<typename T>(T x, T y) { std::cout << x << ' ' << y << '\n'; };
        ...
        {
            auto tl = []<typename... T>(T &&...args) { return foobar(std::forward<T>(args)...); };
