...
namespace recipe_7_09
{
    ...
    execute()
    {
        ...
        {
            auto temp    = path / "dir_with_perm";
            auto success = fs::create_directories(temp, err);
            assert(success);

            // replace permissions with specified ones
            fs::permissions(temp, fs::perms::owner_all | fs::perms::group_all, err);
            if (err)
            {
            ...
            fs::permissions(temp, fs::perms::group_exec, fs::perm_options::remove, err);
            if (err)
            {
                std::cout << err.message() << '\n';
            }
