...
struct std::formatter<recipe_2_17::v1::employee>
{
    ...
    auto
    format(recipe_2_17::v1::employee const &e, std::format_context &ctx) const
    {
...
struct std::formatter<recipe_2_17::v2::employee>
{
    ...
    auto
    format(recipe_2_17::v2::employee const &e, std::format_context &ctx) const
    {
        if (lexicographic_order)
        {
            return std::format_to(ctx.out(), "[{}] {}, {}", e.id, text_format(e.lastName), text_format(e.firstName));
        }

        return std::format_to(ctx.out(), "[{}] {} {}", e.id, text_format(e.firstName), text_format(e.lastName));
    }
  ...
  private:
    ...
    text_format(std::string text) const
    {
        if (lowercase)
        {
...
{
    auto
    format(recipe_2_17::v3::employee const &e, std::format_context &ctx) const
    {
        return std::format_to(ctx.out(), "[{}] {} {}", e.id, e.firstName, e.lastName);
    }
};
