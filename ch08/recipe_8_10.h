    ...
    namespace version2
    {
        ...
        parallel_map(Iter begin, Iter end, F f)
        {
            ...
            else
            {
                ...
                auto result =
                    std::async(std::launch::deferred, parallel_map<Iter, F>, begin, middle, std::forward<F>(f));
        ...
        parallel_fold(Iter begin, Iter end, R init, F op)
        {
            ...
            else
            {
                ...
                auto result1 =
                    std::async(std::launch::async, parallel_fold<Iter, R, F>, begin, middle, R{}, std::forward<F>(op));

                auto result2 = parallel_fold(middle, end, init, std::forward<F>(op));
    ...
    test_mapfold_tasks()
    {
        ...
        for (auto const size : sizes)
        {
            ...
            auto tp1f = perf_timer<>::duration(
                [&] { s2 = version1::parallel_fold(std::begin(v2), std::end(v2), 0LL, std::plus<>()); });
            ...
            auto tp2f = perf_timer<>::duration(
                [&] { s3 = version2::parallel_fold(std::begin(v3), std::end(v3), 0LL, std::plus<>()); });
            ...
            std::cout << std::right << std::setw(8) << std::setfill(' ') << size << std::right << std::setw(8)
                      ...
                      << std::chrono::duration<double, std::micro>(tp2f).count() << '\n';
        }
    }
