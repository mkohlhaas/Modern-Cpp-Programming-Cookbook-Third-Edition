...
namespace details
{
    ...
    struct promise final : public promise_base
    {
        ...
        void
        return_value(V &&value) noexcept(std::is_nothrow_constructible_v<T, V &&>)
        {
            value_ = value;
        }
...
struct task
{
    ...
    struct task_awaiter
    {
        task_awaiter(std::coroutine_handle<promise_type> coroutine) noexcept : handle_(coroutine)
        {
        }
        ...
        void
        await_suspend([[maybe_unused]] std::coroutine_handle<> continuation) noexcept
        {
            handle_.resume();
        }
