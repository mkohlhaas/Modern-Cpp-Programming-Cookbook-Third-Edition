...
main()
{
...

    {
        ...
        if (it != v.cend())
        {
            std::cout << "found " << *it << " at index " << std::ranges::distance(v.cbegin(), it) << '\n';
        }
    }
    ...
    // constrained algorithms + ranges
    {
        ...
        auto range = v | std::views::filter([](int const n) { return n % 2 == 1; }) |
                     std::views::transform([](int const n) { return n * n; }) | std::views::take(4);
    ...
    // projection
    {
        std::vector<Product> products{
            {1, "pen", 15.50}, {2, "pencil", 9.99}, {3, "rubber", 5.0}, {4, "ruler", 5.50}, {5, "notebook", 12.50}};

        auto pos = std::find_if(products.begin(), products.end(), [](Product const &p) { return p.price == 12.5; });
        if (pos != products.end())
        {
            std::cout << pos->name << '\n';
        }
        ...
        std::ranges::for_each(products,
                              [](Product const &p) { std::cout << std::format("{} = {}\n", p.name, p.price); });
    }
}