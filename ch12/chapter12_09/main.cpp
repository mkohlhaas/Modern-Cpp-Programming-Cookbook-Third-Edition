...
struct ranges_ostream_iterator : std::ostream_iterator<T>
{
    ...
    int
    operator-(ranges_ostream_iterator const &)
    {
        return 0;
    }
};
...
class trim_view : public rg::view_interface<trim_view<R, P>>
{
  ...
  public:
    ...
    constexpr trim_view(R base, P pred)
        : base_(std::move(base)), pred_(std::move(pred)), begin_(std::begin(base_)), end_(std::end(base_))
    {
    }
...
namespace details
{
    ...
    template <rg::viewable_range R, typename P>
    constexpr auto
    operator|(R &&r, trim_view_range_adaptor_closure<P> const &a)
    {
...
template <typename P>
void
are_equal(std::vector<int> const &input, std::vector<int> const &output, P &&pred)
{
    std::size_t index = 0;
...
template <typename P>
void
are_equal_reverse1(std::vector<int> const &input, std::vector<int> const &output, P &&pred)
{
    std::size_t index = 0;
...
template <typename P>
void
are_equal_reverse2(std::vector<int> const &input, std::vector<int> const &output, P &&pred)
{
    std::size_t index = 0;
