...
namespace recipe_5_07
{
    ...
    std::vector<color>
    make_gradient(color const &c1, color const &c2, size_t points)
    {
        ...
        std::generate_n(colors.begin(), points, [&r, &g, &b, rstep, gstep, bstep] {
            color c{static_cast<unsigned char>(r), static_cast<unsigned char>(g), static_cast<unsigned char>(b)};
    ...
    execute()
    {
...

        {
            ...
            std::for_each(grayscale.begin(), grayscale.end(), [](color const &c) {
                std::cout << static_cast<int>(c.red) << ", " << static_cast<int>(c.green) << ", "
