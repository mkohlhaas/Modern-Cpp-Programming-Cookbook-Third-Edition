...
namespace recipe_1_15
{
    ...
    namespace v1
    {
        ...
        {
            T &
            operator()(size_t const row, size_t const col)
            {
            ...
            T const &
            operator()(size_t const row, size_t const col) const
            {
                if (row >= ROWS || col >= COLS)
                {
                    throw std::runtime_error("invalid index");
    ...
    namespace v2
    {
        ...
        {
            T &
            operator[](std::initializer_list<size_t> index)
            {
            ...
            T const &
            operator[](std::initializer_list<size_t> index) const
            {
                size_t row = *index.begin();
                size_t col = *(index.begin() + 1);
                if (row >= ROWS || col >= COLS)
