...
namespace recipe_1_06
{
    ...
    */
    ...
    inline void
    print_code(Codes2 const code [[maybe_unused]])
    {
    }
    ...
    execute()
    {
        Codes code [[maybe_unused]] = Codes::Unknown;

        Codes c1 [[maybe_unused]] = Codes::OK; // OK
        // int   c2               = Codes::Failure;                   // error
        int c3 [[maybe_unused]] = static_cast<int>(Codes::Failure); // OK

        {
#ifdef __cpp_using_enum
            using Status::Unknown;
            Status s1 [[maybe_unused]] = Unknown;

            foo::Status s2 [[maybe_unused]] = foo::Created; // instead of foo::Status::Created
#endif
        }
