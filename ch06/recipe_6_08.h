...
namespace recipe_6_08
{
    ...
    make_booking(std::string_view description, int nights, double rate)
    {
        static int nextid = 1;

        return booking{nextid++, nights, rate, description.data(), {}};
    }

    std::optional<booking>
    add_rental(std::optional<booking> b)
    {
    ...
    std::optional<booking>
    add_insurance(std::optional<booking> b)
    {
    ...
    double
    calculate_price(std::optional<booking> b)
    {
        return b->nights * b->rate * 1.1;
    }

    double
    apply_discount(std::optional<double> p)
    {
        return *p * .9;
    }
    ...
    execute()
    {
        ...
        {
            auto b = make_booking("Hotel California", 3, 300).or_else([]() -> std::optional<booking> {
        ...
        {
            auto p = make_booking("Hotel California", 3, 300)
                         .and_then(add_rental)
