...
namespace recipe_6_11
{
    ...
    bool
    IsValidFormat(Image const &img)
    {
        return true;
    }
    bool
    IsValidLength(Image const &img)
    {
        return true;
    }
    bool
    Transform(Image &img, Filter const filter)
    {
        switch (filter)
        {
...
#ifdef __cpp_lib_expected
    std::expected<Image, Status>
    ApplyFilter(Image img, Filter const filter)
    {
    ...
    execute()
    {
...
#if __cpp_lib_expected == 202211L
        {
            ...
            ApplyFilter(img, Filter::Sepia)
                .and_then([](Image result) { return ApplyFilter(result, Filter::Pixelize); })
                .and_then([](Image result) { return ApplyFilter(result, Filter::Blur); })
...

        {
            ...
            auto obj =
                ApplyFilter(img, Filter::Sepia)
                    ...
                    .transform_error([](Status status) { return status == Status::Success ? "success" : "fail"; });
