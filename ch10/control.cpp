...
namespace recipe_10_02
{
    ...
    void
    control::set_text(std::string_view text)
    {
        pimpl->set_text(text);
    }

    void
    control::resize(int const w, int const h)
    {
        pimpl->resize(w, h);
    }
    ...
    control_copyable::control_copyable() : pimpl(new control_pimpl(), [](control_pimpl *pimpl) { delete pimpl; })
    {
    }
    ...
    void
    control_copyable::set_text(std::string_view text)
    {
        pimpl->set_text(text);
    }

    void
    control_copyable::resize(int const w, int const h)
    {
        pimpl->resize(w, h);
    }

    void
    control_copyable::show()
    {
        pimpl->show();
    }

    void
    control_copyable::hide()
    {
        pimpl->hide();
    }
} // namespace recipe_10_02