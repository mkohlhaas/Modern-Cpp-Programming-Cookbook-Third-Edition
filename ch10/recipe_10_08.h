...
namespace recipe_10_08
{
    ...
    namespace v1
    {
        ...
        {
            template <typename T>
            control(T &&obj) : ctrl(std::make_shared<control_model<T>>(std::forward<T>(obj)))
            {
            }
        ...
        void
        draw(std::vector<control> &controls)
        {
            for (auto &c : controls)
            {
                c.draw();
    ...
    namespace v3
    {
        ...
        void
        draw(std::vector<control_concept *> const &controls)
        {
            for (auto &c : controls)
            {
                c->draw();
