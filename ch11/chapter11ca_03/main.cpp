...
namespace v1
{
    ...
    inline OnTheLine
    IsOnTheLine(point3d const &p1, point3d const &p2)
    {
...
namespace v2
{
    class OnTheLine : public Catch::Matchers::MatcherGenericBase
    {
      ...
      protected:
        ...
        {
#ifdef __cpp_lib_format
            return std::format("on the line between ({},{},{}) and ({},{},{})", p1.x(), p1.y(), p1.z(), p2.x(), p2.y(),
                               p2.z());
#else
            std::ostringstream ss;
            ss << "on the line between " << p1 << " and " << p2;
...
TEST_CASE("assertions")
{
    ...
    SECTION("matchers")
    {
        ...
        CHECK_THAT(text, Catch::Matchers::ContainsSubstring("EXAMPLE", Catch::CaseSensitive::No));
        REQUIRE_THAT(text, Catch::Matchers::StartsWith("this") && Catch::Matchers::ContainsSubstring("an"));
    }
}
