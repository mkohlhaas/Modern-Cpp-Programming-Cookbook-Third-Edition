    ...
    namespace Benchmark
    {
        namespace Detail
        {
            namespace
            {
                ...
                static sample
                resample(URng &rng, unsigned int resamples, std::vector<double>::const_iterator first,
                         std::vector<double>::const_iterator last, Estimator &estimator)
                {
                ...
                static double
                standard_deviation(std::vector<double>::const_iterator first, std::vector<double>::const_iterator last)
                {
...
#pragma GCC diagnostic pop
            ...
            classify_outliers(std::vector<double>::const_iterator first, std::vector<double>::const_iterator last)
            {
            ...
            double
            mean(std::vector<double>::const_iterator first, std::vector<double>::const_iterator last)
            {
                auto   count = last - first;
                double sum   = 0.;
                while (first != last)
...
#if defined(CATCH_CONFIG_USE_ASYNC)
                auto Estimate =
                    [=](double (*f)(std::vector<double>::const_iterator, std::vector<double>::const_iterator)) {
                        auto seed = entropy();
...
namespace Catch
{
    ...
    namespace
    {
        ...
        static Optional<bazelShardingOptions>
...
namespace Catch
{
    ...
    TestCaseInfo::TestCaseInfo(StringRef _className, NameAndTags const &_nameAndTags, SourceLineInfo const &_lineInfo)
        : name(_nameAndTags.name.empty() ? makeDefaultName() : _nameAndTags.name), className(_className),
          lineInfo(_lineInfo)
    {
        ...
        for (size_t idx = 0; idx < originalTags.size(); ++idx)
        {
            ...
            if (c == '[')
            {
                CATCH_ENFORCE(!inTag, "Found '[' inside a tag while registering test case '" << _nameAndTags.name
                                                                                             << "' at " << _lineInfo);
        ...
        CATCH_ENFORCE(!inTag, "Found an unclosed tag while registering test case '" << _nameAndTags.name << "' at "
                                                                                    << _lineInfo);
...
namespace Catch
{
    ...
    void
    TestSpec::NamePattern::serializeTo(std::ostream &out) const
    {
        out << '"' << name() << '"';
    }
    ...
    void
    TestSpec::TagPattern::serializeTo(std::ostream &out) const
    {
        out << name();
    }
    ...
    void
    TestSpec::Filter::serializeTo(std::ostream &out) const
    {
        bool first = true;
        for (auto const &pattern : m_required)
        {
    ...
    std::string
    TestSpec::extractFilterName(Filter const &filter)
    {
    ...
    void
    TestSpec::serializeTo(std::ostream &out) const
    {
        bool first = true;
        for (auto const &filter : m_filters)
        {
...
namespace Catch
{
    ...
    {
        void
        registerTranslatorImpl(Detail::unique_ptr<IExceptionTranslator> &&translator)
        {
...
namespace Catch
{
    ...
    SectionStats::SectionStats(SectionInfo &&_sectionInfo, Counts const &_assertions, double _durationInSeconds,
        ...
        : sectionInfo(CATCH_MOVE(_sectionInfo)), assertions(_assertions), durationInSeconds(_durationInSeconds),
    ...
    TestCaseStats::TestCaseStats(TestCaseInfo const &_testInfo, Totals const &_totals, std::string &&_stdOut,
        ...
        : testInfo(&_testInfo), totals(_totals), stdOut(CATCH_MOVE(_stdOut)), stdErr(CATCH_MOVE(_stdErr)),
          aborting(_aborting)
    {
    }
...
namespace Catch
{
    ...
    void
    Context::setConfig(IConfig const *config)
    {
        m_config = config;
    }
...
namespace Catch
{
    ...
    void
    ExceptionTranslatorRegistry::registerTranslator(Detail::unique_ptr<IExceptionTranslator> &&translator)
    {
...
namespace Catch
{
    ...
    parseUInt(std::string const &input, int base)
    {
...
namespace Catch
{
    ...
    ReporterSpec::ReporterSpec(std::string name, Optional<std::string> outputFileName, Optional<ColourMode> colourMode,
        ...
        : m_name(CATCH_MOVE(name)), m_outputFileName(CATCH_MOVE(outputFileName)), m_colourMode(CATCH_MOVE(colourMode)),
          m_customOptions(CATCH_MOVE(customOptions))
    {
    }
...
namespace Catch
{
    ...
    namespace Generators
    {
        namespace
        {
            struct GeneratorTracker final : TestCaseTracking::TrackerBase, IGeneratorTracker
            {
                ...
                acquire(TrackerContext &ctx, TestCaseTracking::NameAndLocationRef const &nameAndLocation)
                {
    ...
    void
    RunContext::assertionEnded(AssertionResult &&result)
    {
    ...
    void
    RunContext::notifyAssertionStarted(AssertionInfo const &info)
    {
        m_reporter->assertionStarting(info);
    }

    bool
    RunContext::sectionStarted(StringRef sectionName, SourceLineInfo const &sectionLineInfo, Counts &assertions)
    {
                                       ...
                                       Generators::GeneratorBasePtr &&generator)
    {
        ...
        auto newTracker = Catch::Detail::make_unique<Generators::GeneratorTracker>(CATCH_MOVE(nameAndLoc),
                                                                                   m_trackerContext, &currentTracker);
    ...
    void
    RunContext::sectionEnded(SectionEndInfo &&endInfo)
    {
    ...
    void
    RunContext::sectionEndedEarly(SectionEndInfo &&endInfo)
    {
    ...
    void
    RunContext::emplaceUnscopedMessage(MessageBuilder &&builder)
    {
    ...
    void
    RunContext::handleMessage(AssertionInfo const &info, ResultWas::OfType resultType, StringRef message,
    ...
    void
    RunContext::handleUnexpectedInflightException(AssertionInfo const &info, std::string &&message,
                                                  AssertionReaction &reaction)
    {
        m_lastAssertionInfo = info;
...
namespace Catch
{
    namespace TestCaseTracking
    {
        ...
        SectionTracker &
        SectionTracker::acquire(TrackerContext &ctx, NameAndLocationRef const &nameAndLocation)
        {
...
#pragma clang diagnostic pop
        ...
        bool
        IsNaNMatcher::match(double const &matchee) const
        {
            return std::isnan(matchee);
        }

        std::string
        IsNaNMatcher::describe() const
        {
            using namespace std::string_literals;
            return "is NaN"s;
        }
...
namespace Catch
{
    ReporterBase::ReporterBase(ReporterConfig &&config)
        : IEventListener(config.fullConfig()), m_wrapped_stream(CATCH_MOVE(config).takeStream()),
          m_stream(m_wrapped_stream->stream()), m_colour(makeColourImpl(config.colourMode(), m_wrapped_stream.get())),
          ...
          public:
            ...
            AssertionPrinter(std::ostream &_stream, AssertionStats const &_stats, bool _printInfoMessages,
                ...
                : stream(_stream), result(_stats.assertionResult), messages(_stats.infoMessages),
                  itMessage(_stats.infoMessages.begin()), printInfoMessages(_printInfoMessages), colourImpl(colourImpl_)
            {
            }
...
namespace Catch
{
          ...
          public:
            ...
            ConsoleAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats, ColourImpl *colourImpl_,
                ...
                : stream(_stream), stats(_stats), result(_stats.assertionResult), colour(Colour::None),
                  messages(_stats.infoMessages), colourImpl(colourImpl_), printInfoMessages(_printInfoMessages)
            {
                switch (result.getResultType())
                {
                case ResultWas::Ok:
    ...
    class TablePrinter
    {
      ...
      public:
        ...
        operator<<(TablePrinter &tp, ColumnBreak)
        {
            ...
            auto padding =
                (strSize + 1 < colInfo.width) ? std::string(colInfo.width - (strSize + 1), ' ') : std::string();
    ...
    ConsoleReporter::benchmarkPreparing(StringRef name)
    {
        ...
        auto nameCol =
            TextFlow::Column(static_cast<std::string>(name)).width(m_tablePrinter->columnInfos()[0].width - 2);
...
namespace Catch
{
    ...
    void
    defaultListReporters(std::ostream &out, std::vector<ReporterDescription> const &descriptions, Verbosity verbosity)
    {
    ...
    void
    defaultListTests(std::ostream &out, ColourImpl *streamColour, std::vector<TestCaseHandle> const &tests,
    ...
    namespace
    {
        ...
        void
        printSummaryRow(std::ostream &stream, ColourImpl &colour, StringRef label,
                        std::vector<SummaryColumn> const &cols, std::size_t row)
        {
            for (auto const &col : cols)
            {
                ...
                {
                    stream << colour.guardColour(Colour::LightGrey) << " | " << colour.guardColour(col.getColour())
                           << value << ' ' << suffix;
                }
            }
            stream << '\n';
    ...
    void
    printTestRunTotals(std::ostream &stream, ColourImpl &streamColour, Totals const &totals)
    {
        if (totals.testCases.total() == 0)
        {
...
namespace Catch
{
    ...
    void
    JunitReporter::writeSection(std::string const &className, std::string const &rootName,
...
namespace Catch
{
    namespace Detail
    {
        ...
        void
        registerListenerImpl(Detail::unique_ptr<EventListenerFactory> listenerFactory)
        {
...
namespace Catch
{
    ...
    {
        std::string
        createMetadataString(IConfig const &config)
        {
            ReusableStringStream sstr;
    ...
    void
    SonarQubeReporter::writeTestFile(StringRef filename, std::vector<TestCaseNode const *> const &testCaseNodes)
    {
    ...
    void
    SonarQubeReporter::writeSection(std::string const &rootName, SectionNode const &sectionNode, bool okToFail)
    {
        std::string name = trim(sectionNode.stats.sectionInfo.name);
        if (!rootName.empty())
        {
